//
//  WBYHostilJiGouViewController.m
//  whm_project
//
...
@interface WBYHostilJiGouViewController ()<BMKMapViewDelegate,BMKLocationServiceDelegate,UIActionSheetDelegate>
{
    BMKMapView* _mapView;
...
@implementation WBYHostilJiGouViewController

- (void)viewDidLoad
{
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationItem.title = @"医院位置";
    [self creatLeftTtem];
...
-(void)creatmap
{
    ...
    //    [UIImage imageNamed:@"tel"];
    self.mapLaber=[UILabel new];
    self.mapLaber.frame = CGRectMake(CGRectGetMaxX(self.mapImg.frame)+3, CGRectGetMaxY(self.addressLaber.frame),40, litH);
    self.mapLaber.textColor = [UIColor greenColor];
    self.mapLaber.font = [UIFont systemFontOfSize:14.0];
    [self.myView addSubview:_mapLaber];
    self.mapLaber.text = self.myDataModel.distance.length>2?self.myDataModel.distance:@"";
    ...
    self.telLab.text = self.myDataModel.tel.length>5?self.myDataModel.tel:@"暂无电话";
...
-(void)rodeButAction
{
    if (_myDataModel.latitude.length < 3 || _myDataModel.longitude.length < 3)
    {
