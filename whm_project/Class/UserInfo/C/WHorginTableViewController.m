...
#import "MJRefresh.h"
#import "WBYfenzhijigouViewController.h"
...
-(void)aa:(UIBarButtonItem *)sender
{
    ...
    WBYfenzhijigouViewController * orgList = [[WBYfenzhijigouViewController alloc] init];
...
-(void)myCategoryBtnAction
{
    ...
    self.companyTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, CGRectGetWidth(_companyBackView.frame), CGRectGetHeight(self.view.frame) ) style:UITableViewStylePlain];
...
//        WHorgMapTableViewController
        WBYFzljghViewController * orgMap = [[WBYFzljghViewController alloc]init];
        DataModel * model = self.dataArry[indexPath.row];
//        orgMap.p_Name = model.name;
//        orgMap.p_Address = model.address;
//        orgMap.p_mapLaber = model.distance;
//        orgMap.p_Mobile = model.tel;
        
        orgMap.myDataModel = model;
...
-(void)telAction:(UIButton *)sender
{
   WHorganization * orgin = self.dataArry[sender.tag - 100];
    self.tel = orgin.tel;
    NSString * s1 = [NSString stringWithFormat:@"你确定要拨打电话吗?"];
    NSString * s2 = [s1 stringByAppendingString:self.tel];
    
    UIAlertView *view = [[UIAlertView alloc]initWithTitle:@"温馨提示" message:s2  delegate:self cancelButtonTitle:@"确定" otherButtonTitles:@"取消", nil];
